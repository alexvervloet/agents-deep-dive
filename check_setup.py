...
PROVIDER_DEPS = {
    ...
    "claude": [("anthropic", "anthropic", "Claude messages + tool use")],
}
PROVIDER_KEYS = {
...
def check_keys(env, provider):
    ...
    for name, prefix, placeholder in PROVIDER_KEYS.get(provider, []):
        ...
        if not value or value == placeholder:
            ...
            print("    Store it in your OS keychain and run `secrun python check_setup.py` . See ../docs/SECRETS.md.")
            all_ok = False
...
def main():
    print(_c("Checking your setup for the Agents deep dive...\n", "1"))
    env = _read_env_file()
    py = check_python()
    ...
    if py and deps and keys:
        ...
        print("(Examples 01, 07, 10, and 18 are offline and need no key.)")
        return 0
    print(_c("Not ready yet. Fix the ✗ items above, then run this again.", "1;31"))
    print("(Examples 01, 07, 10, and 18 are offline, so you can run those without a key.)")
    return 1
